...
`list?` must return `#f` for all improper lists, including cyclic lists. A definition of `list?` is shown on <<defn:list?,page 67>>.
...
`length` may be defined as follows, using an adaptation of the hare and tortoise algorithm used for the definition of `list?` on <<defn:list?,page 67>>.
...
[#page:assq]
[horizontal]
procedure:: `(assq _obj_ _alist_)` [[objects:s58]]
...
Complex numbers may be written in either rectangular or polar form. In rectangular form, a complex number is written as `__x__+__y__i` or `__x__-__y__i`, where `_x_` is an integer, rational, or real number and `_y_` is an unsigned integer, rational, or real number. The real part, _x_, may be omitted, in which case it is assumed to be zero. For example, `3+4i`, `3.2-3/4i`, `+i`, and `-3e-5i` are complex numbers written in rectangular form. In polar form, a complex number is written as `_x_@_y_`, where `_x_` and `_y_` are integer, rational, or real numbers. For example, `1.1@1.764` and `-1@-1/2` are complex numbers written in polar form.
...
returns:: the integer closest to `_real_` toward stem:[-\infty]
libraries:: `(rnrs base)`, `(rnrs)`
...
[#page:abs]
[horizontal]
procedure:: `(abs _real_)` [[objects:s105]]
...
[#page:gcd]
[horizontal]
procedure:: `(gcd _int_ \...)` [[objects:s109]]
...
The result is always nonnegative, i.e., common multiples of -1 are ignored. Although `lcm` should probably return stem:[\infty] when called with no arguments, it is defined to return 1. If one or more of the arguments is 0, `lcm` returns 0.
...
`rationalize` returns the simplest rational number that differs from `_real~1~_` by no more than `_real~2~_`. A rational number stem:[q_1 = n_1/m_1] is simpler than another rational number stem:[q_2 = n_2/m_2] if stem:[|n_1| \le |n_2|] and stem:[|m_1| \le |m_2|] and either stem:[|n_1| \lt |n_2|] or stem:[|m_1| \lt |m_2|].
...
[#page:magnitude]
[horizontal]
procedure:: `(magnitude _num_)` [[objects:s125]]
returns:: the magnitude of `_num_`
libraries:: `(rnrs base)`, `(rnrs)`

`magnitude` and `abs` (see <<page:abs,page 178>>) are identical for real arguments. The magnitude of a complex number stem:[x + yi] is stem:[+\sqrt{x^2+y^2}].
...
This procedure returns two nonnegative exact integers _s_ and _r_ where stem:[n = s^2 + r] and stem:[n < (s + 1)^2].
...
This section describes the operations that deal primarily with characters. See also the following section on strings and <<chp_7,Chapter 7>> on input and output for other operations relating to characters.
