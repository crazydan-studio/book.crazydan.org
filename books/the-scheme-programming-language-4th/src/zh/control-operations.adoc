...
[#desc:apply]
[horizontal]
procedure:: `(apply _procedure_ _obj_ \... _list_)` [[control:s3]]
...
[#desc:for-each]
[horizontal]
procedure:: `(for-each _procedure_ _list~1~_ _list~2~_ \...)` [[control:s33]]
returns:: unspecified
libraries:: `(rnrs base)`, `(rnrs)`
...
`environment` returns an environment formed from the combined bindings of the given import specifiers. Each `_import-spec_` must be an s-expression representing a valid import specifier (see <<chp_10,Chapter 10>>).
