...
// document.querySelectorAll('h4').forEach(function(node) { title=node.innerText; anchor='exercise_'+title.replace(/^Exercise ([\d.]+)$/, '$1'); node.outerHTML = '==== ' + title + ' [['+anchor+']]<br>'; })
...
// document.querySelectorAll('sup').forEach(function(node) { node.outerHTML = '^'+node.innerText+'^' })

// Replacements
// - ^$\n`\s*\(     \n[source,scheme,subs=""]\n----\n(
// - \)`\n^$     )\n----\n
// - \[(\d+)\]   <<ref$1>>
// - ([^=]) Section ([\d\.]+\d)    $1 <<section_$2.,Section $2>>
// - ([^=]) Exercise ([\d\.]+\d)    $1 <<exercise_$2,Exercise $2>>
// - ([(])Section ([\d\.]+\d)    $1<<section_$2.,Section $2>>
