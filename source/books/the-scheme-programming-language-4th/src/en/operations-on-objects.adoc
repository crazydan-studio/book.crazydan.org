...
The grammar for symbols on <<grammar_symbols,page 458>> gives a precise definition of the syntax of symbols.
...
Hashtables represent sets of associations between arbitrary Scheme values. They serve essentially the same purpose as association lists (see <<page_assq,page 165>>) but are typically much faster when large numbers of associations are involved.
